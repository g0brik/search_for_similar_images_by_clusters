...
# np.save('all_pic', all_pic)
# all_pic = np.load('all_pic.npy')
# np.save('km_ap', km_ap)
# km_ap = np.load('km_ap.npy', allow_pickle=True)
# np.save('a_ap', a_ap)
# a_ap = np.load('a_ap.npy')


# if os.path.exists('km_ap.npy') is True:
#     # km_ap = np.load('km_ap.npy',allow_pickle=True)
#     km_ap = wget.download(km_ap, 'km_ap.npy')
# else:
#     all_pic, km_ap, a_ap = cluster_img('imgs/*.*')
#
# if os.path.exists('all_pic.npy') is True:
#     all_pic = np.load('all_pic.npy')
# else:
#     all_pic, km_ap, a_ap = cluster_img('imgs/*.*')
#
# if os.path.exists('a_ap.npy') is True:
#     # km_ap = np.load('km_ap.npy',allow_pickle=True)
#     a_ap = np.load('a_ap.npy')
# else:
#     all_pic, km_ap, a_ap = cluster_img('imgs/*.*')
...
Sigma_main_picture = sigma(a_mp, km_mp)  # number of pixels in the main image in cluster
    ...
    for y, sap in enumerate(Sigma_all_pic):
        ...
        for id9, s1 in enumerate(smp):  # consider Smax/Smin(s-number of pixels in each cluster)=sigma and get the matrix
            ...
            for ii, s2 in enumerate(sap):
                ...
                if ii == len(sap):
                    break
            list_ss_matrix.append(list_ss)  # matrix 2 imdgs 5x5
            if id9 == len(smp):
                break
        if y == len(Sigma_all_pic):
            break
...
for id7, m_p in enumerate(main_picture):
    for ii, pici in enumerate(all_pic):  # we find the study m \ y the main picture and the rest of the population DELTA
        ...
        for i in range(0, int(clusters_n)):
            for j in range(0, int(clusters_n)):
                d[i][j] = euclidean_distance(m_p[i], pici[j])
        # d = euclidean_distance(m_p, pici)
        dd.append(d)
        if ii == len(all_pic):
            break
    # dd.append(d1)
    if id7 == len(main_picture):
        break
...
for id3, el in enumerate(DxS_list):  # find pairs in the matrix by the minimum values of the product delta x sigma
    ...
    index_s = []
    #i1 == 0
    for i1 in range(0, int(clusters_n)):
        y = np.amin(el)
        ...
        index_s.append([index_y, index_x])
        if i1 == len(range(0, int(clusters_n))):
            break
...
print('list_min_sigma=', list_min_sigma)
list_total = []
for id2, s1 in enumerate(Sigma_main_picture):    # we calculate the sum of c1 and c2 10 pairs (which have the minimum sigma by index)
    for s2, ind in zip(Sigma_all_pic, list_min_sigma_index):
        total_s = []
        for f, index in enumerate(ind):
            total_el = s1[int(index[0])]+s2[int(index[1])]
            total_s.append(int(total_el))
            if f == len(ind):
                break
        list_total.append(total_s)
    if id2 == len(Sigma_main_picture):
        break
...
list_result = []
for id6, summa in enumerate(list_total):
    result = summa/524288
    list_result.append(result)
    if id6 == len(list_total):
        break

list_1_x = []
for sig, sf in zip(list_min_sigma, list_result):  # multiply "softmax" by the product of delta by sigma
    list_dxsxsm = []
    for x, y in zip(range(0, int(clusters_n)), range(0, int(clusters_n))):
        dxsxsm = sig[x] * sf[y]
...
viz_g = np.concatenate((resized_image_same, resized_image_mean, resized_image_diff), axis=0)   # glue pictures in 1
